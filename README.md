...
- 💬 **AI智能助手**: 支持流式对话和多轮聊天，类似ChatGPT体验
- 🏥 **系统健康监控**: 全面的外部服务健康检查和状态监控
...
### AI聊天助手
1. 在主界面点击"AI助手"按钮或直接访问 `/chat`
2. 输入问题或选择快速问题开始对话
3. 支持多轮对话和实时流式回复
4. 可随时清空对话历史重新开始

### 系统健康监控
- **完整检查**: `GET /api/health/check` - 检查所有外部服务端点
- **快速状态**: `GET /api/health/status` - 快速获取关键服务状态
- 返回详细的健康报告，包括响应时间、状态码等信息
...
#### 聊天接口
- `POST /api/chat/message` - AI聊天对话（需认证）
- `POST /api/chat/stream` - 流式AI聊天（需认证）
- `GET /api/chat/test` - 测试聊天服务连接

#### 健康检查接口
- `GET /api/health/check` - 完整系统健康检查
- `GET /api/health/status` - 快速健康状态查询
