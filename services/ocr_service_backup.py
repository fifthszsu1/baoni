...
class OCRService:
    """OCR图片文字识别服务（支持Tesseract和Azure OpenAI回退）"""
    ...
    def __init__(self):
        ...
        # 尝试初始化Azure OpenAI作为备选
        try:
            from services.openai_service import OpenAIService
            self.openai_service = OpenAIService()
            logger.info("Azure OpenAI OCR备选方案可用")
            self.openai_available = True
        except Exception as e:
            logger.warning(f"Azure OpenAI OCR备选方案不可用: {str(e)}")
            self.openai_service = None
            self.openai_available = False
        
        # 设置总体可用性
        self.available = self.tesseract_available or self.openai_available
        
        if not self.available:
            logger.error("所有OCR服务都不可用")
        else:
            primary_service = "Tesseract" if self.tesseract_available else "Azure OpenAI"
            logger.info(f"OCR服务初始化完成，主要使用: {primary_service}")
